...
//    getNotes(getIntent().getStringExtra("code"));
  ...
  private void setNotesFirebase(final String subjectCode){
      DatabaseReference ref= FirebaseDatabase.getInstance().getReference().child("notes").child(subjectCode);
      FirebaseRecyclerAdapter<NotesFileModel,NotesFileAdapter.MyHolder> adapter=new FirebaseRecyclerAdapter<NotesFileModel, NotesFileAdapter.MyHolder>(NotesFileModel.class,
              R.layout.card_view_notes_subjects_item, NotesFileAdapter.MyHolder.class,
              ref) {
          @Override
          protected void populateViewHolder(NotesFileAdapter.MyHolder viewHolder, NotesFileModel model, int position) {
  ...
  // Requesting permission
  @AfterPermissionGranted(RC_STORAGE_PERMISSION) private void requestStoragePermission(String type) {
