    ...
    new Handler().postDelayed(new Runnable() {
      ...
      public void run() {
        if (AppController.getInstance().getPrefManager().getIsFirstRun()) {
          ...
          finish();
        } else if (mFirebaseUser != null && AppController.getInstance()
            .getPrefManager()
            .isLoggedIn()) {
          ...
          @Override public void run() {
            if (windowDetached) {
              ...
              if (android.os.Build.VERSION.SDK_INT > Build.VERSION_CODES.LOLLIPOP) {
                ...
                Animator animator =
                    ViewAnimationUtils.createCircularReveal(layoutContent, cx, cy, 0, finalRadius);
                ...
                layoutContent.setVisibility(View.VISIBLE);
                layoutContent.animate().alpha(1f).setDuration(1000).setListener(null);
              }
            }
          }
        }, 330);
